...
contract ENSBaseRegistrar is BaseRegistrar {
    ...
    constructor(ENS _ens, HashRegistrar _previousRegistrar, bytes32 _baseNode, uint _transferPeriodEnds) public {
        ...
        transferPeriodEnds = 0; // Modified line. Should be _transferPeriodEnds originally
    }
    ...
    function register(uint256 id, address owner, uint duration) external live onlyController returns(uint) {
        ...
        require(now + duration + GRACE_PERIOD > now + GRACE_PERIOD, "wrong period"); // Prevent future overflow
