...
contract DCLRegistrar is ERC721Full, Ownable {
    ...
    event NameRegistered(address indexed _caller, address indexed _beneficiary, string _subdomain);
    ...
    event BaseUpdated(IBaseRegistrar indexed previousBase, IBaseRegistrar indexed newBase);
    ...
    function migrateNames(bytes32[] calldata _names, address[] calldata _beneficiaries) external onlyOwner isMigrating {
        for (uint256 i = 0; i < _names.length; i++) {
            ...
            _register(name, subdomainLabelHash, _beneficiaries[i]);
        }
    }
    ...
    function _register(string memory _subdomain, bytes32 subdomainLabelHash, address _beneficiary) internal {
        ...
        // Emit subdomain creation event
        emit NameRegistered(msg.sender, _beneficiary, _subdomain);
    }
    ...
    function reclaim(uint256 _tokenId) public {
         // Check if the sender is owner of the subdomain
        require(
    ...
    */
    ...
    function available(bytes32 _labelhash) public returns (bool) {
        // Make sure it is free
        return registry.owner(_labelhash) == address(0) &&
            ownerOf(uint256(_labelhash)) == address(0);
    }
